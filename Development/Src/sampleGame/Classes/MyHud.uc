...
simulated event PostBeginPlay() //Al empezar
{
    ...
    MyHudHealth.Start();
    MyHudHealth.UpdateLife(SamplePlayerController(PlayerOwner).Pawn.Health);

    GameMinimap = SampleGame(WorldInfo.Game).GameMinimap;
}
...
event PostRender()
{
    local SamplePlayerCamera PlayerCam;
    local SamplePlayerController IsoPlayerController;
    local int TeamIndex;
    ...
    IsoPlayerController = SamplePlayerController(PlayerOwner); // Cast de la clase playerController para obtener el jugador
    ...
    PlayerCam = SamplePlayerCamera(IsoPlayerController.PlayerCamera); //Obtenemos la camara con cast
...
{
    local SamplePlayerController IsoPlayerController;
    IsoPlayerController = SamplePlayerController(PlayerOwner);
...
function DrawHUD()
{
    ...
    if(bDrawTraces) //Si bDrawTraces == true se dibujan
    {
        ...
        if(SamplePlayerController(PlayerOwner).TraceActor != none)
        {
            StringMessage = "Actor selected:"@SamplePlayerController(PlayerOwner).TraceActor.class;
        }
